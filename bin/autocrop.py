    ...
    if found:
        ...
        try:
            if black_bg:
                img = ~img
            cv2.imwrite(
                f"{out_path}/{name}", img, [int(cv2.IMWRITE_JPEG_QUALITY), 100]
            )
        except:
            None
        ...
        print(
            f"Failed finding any contour. Saving original file to {out_path}/failed/{name}"
        )
        if not os.path.exists(f"{out_path}/failed/"):
            os.makedirs(f"{out_path}/failed/")

        with open(filename, "rb") as in_f, open(
            f"{out_path}/failed/{name}", "wb"
        ) as out_f:
            while True:
...
def main():
    parser = argparse.ArgumentParser(
        description="Crop/Rotate images automatically. Images should be single images on white background."
    )
    parser.add_argument(
        "-i",
        metavar="INPUT_PATH",
    ...
    in_path = pathlib.PureWindowsPath(
        args.i
    ).as_posix()  # since windows understands posix too: let's convert it to a posix path.
    out_path = pathlib.PureWindowsPath(
        args.o
    ).as_posix()  # (works on all systems and conveniently also removes additional '/' on posix systems)
    ...
    if not single:
        types = (
            ...
            "*.png",
            "*.PNG",
        )  # all should work but only .jpg was tested
    ...
    if len(files) == 0:
        ...
        if num_threads == None:
            try:
                ...
                print(
                    "Automatic thread detection didn't work. Defaulting to 1 thread only. \
