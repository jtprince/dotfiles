...
def each_cons(sequence, n):
    return izip(*(islice(g, i, None) for i, g in enumerate(tee(sequence, n))))
...
class Comment(object):
    ...
    def docstring_needed(self):
        return (
            not self.has_docstring()
            and not any(substr in self.defline for substr in self.NO_DOCSTRINGS)
            and not self.docstring_added
        )
        ...
        if self.already_made_docstring:
            ...
            if self.has_docstring():
                if self.is_singleline():
                    docstr = (
                        self.left_padding(self.topline)
                        + '""" '
                        + self.message_capitalized_and_puncuated()
                        + ' """\n'
                        + self.subline
                    )
                ...
                if self.is_testfile() and self.docstring_needed():
                    if "def setup(" in self.defline or "def setUp(" in self.defline:
                        ...
                        print(self.subline)
                        docstr = (
                            self.left_padding(self.defline)
                            + "    "
                            + '""" Setup. """\n'
                            + self.topline
                            + self.subline
    ...
    def is_bonafide_docstring(self):
        return any(keyword in self.defline for keyword in ["def ", "class "]) and (
            "(" in self.defline
        )
...
parser.add_argument(
    "--open", action="store_true", help="open in gvim the files missing doc strings"
)
...
for path in args.paths:
    ...
    for comment in comments:
        ...
        if comment.docstring_needed():
            ...
            sys.stderr.write(
                '    File "%s", line %d' % (path, comment.linenum + 1) + "\n"
            )
