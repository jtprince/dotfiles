...
parser.add_argument(
    "issues",
    nargs="+",
    help="the issues.  Make sure to include prefix (like 'WMS-123')",
)
parser.add_argument(
    "--api-key",
    default=os.environ.get("JIRA_API_KEY"),
    help="defaults to env var JIRA_API_TOKEN",
)
...
def get_histories(histories, transition):
    """Returns list of all history dicts with transitions specified by from and to."""
    return [
        history
...
for issue in args.issues:
    ...
    response = requests.get(
        url, params=dict(expand="changelog"), auth=(args.login, args.api_key)
    )
    data = response.json()
