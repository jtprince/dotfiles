...
parser = argparse.ArgumentParser(description="grep the subject lines of the various cron jobs and return the info")
parser.add_argument("pattern", help="the regex pattern to search for")
parser.add_argument("-f", "--filename", default="/var/spool/mail/root" help="the qualified path of the cronfile")
...
matching_cron_stati = [status for status in cron_stati if match_subj_re.search(status.subject)]
