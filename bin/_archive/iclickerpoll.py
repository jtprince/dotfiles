...
class Command(object):
    def __init__(self, byte_array=None):
        ...
        if type(byte_array) is str:
            ...
            byte_array = array(
                "B",
                (int(byte_array[i : i + 2], 16) for i in range(0, len(byte_array), 2)),
            )
    ...
    def __repr__(self):
        ...
        return " ".join(
            hex_string[i : i + SPLIT_BY_N_CHARS]
            for i in range(0, len(hex_string), SPLIT_BY_N_CHARS)
        ).lower()
...
class IClickerBase(object):
    ...
    def _syncronous_write(self, data, timeout=100):
        ...
        if response != expected_response:
            raise IOError(
                "Attempted syncronuous write of {0} and got {1} (expecting {2})".format(
                    data.__repr__(), response.__repr__(), expected_response.__repr__()
                )
            )
    ...
    def get_base(self):
        ...
        with self.usb_lock:
            ...
            if self.device.is_kernel_driver_active(0):
                log.warning(
                    "The iClicker seems to be in use by another device--Forcing reattach."
                )
    ...
    def set_base_frequency(self, code1="a", code2="a"):
        ...
        time.sleep(0.2)
        cmd = Command(
            [0x01, 0x10, 0x21 + code_to_number(code1), 0x41 + code_to_number(code2)]
        )
    ...
    def set_screen(self, string, line=0, force_update=False):
        ...
        def process_screen_queue(line):
            ...
            if curr_time - self.last_set_screen_time < MIN_SCREEN_UPDATE_TIME:
                delay_duration = MIN_SCREEN_UPDATE_TIME - (
                    curr_time - self.last_set_screen_time
                )
...
class Response(object):
    ...
    def __eq__(self, other):
        if type(other) is Response:
            return (
                self.clicker_id == other.clicker_id
    ...
    def __ne__(self, other):
        if type(other) is Response:
            return (
                self.clicker_id != other.clicker_id
                or self.response != other.response
                or self.seq_num != other.seq_num
            )
        else:
            return True
    ...
    def __repr__(self):
        return "{0}: {1} ({2} at {3})".format(
            self.clicker_id, self.response, self.seq_num, self.click_time
        )
...
class IClickerPoll(object):
    ...
    def update_display(self):
        ...
        tally = Counter(
            r.response for r in recent_responses if r.response != "F"
        )  #'F' means retract answer
        if len(tally) > 0:
            total = sum(tally.values())
            out_string = "{0} {1} {2} {3} {4}".format(
                int(100 * tally["A"] / total),
        ...
        out_string = "{0}{1:>{padding}}".format(
            out_string_time, sum(tally.values()), padding=(16 - len(out_string_time))
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dest", type=str, default="", help="Sets the file to save polling data to."
    )
    parser.add_argument(
        "--frequency",
        type=str,
    ...
    if args.type in ["alpha", "numeric", "alphanumeric"]:
        ...
        raise ValueError(
            "Poll type must be 'alpha', 'numeric', or 'alphanumeric', not '{0}'".format(
                args.type
            )
        )
    if args.duration:
    ...
    if args.frequency:
        ...
        if freq1 not in ("a", "b", "c", "d") or freq2 not in ("a", "b", "c", "d"):
            raise ValueError(
                "Frequency combintation '{0}{1}' is not valid".format(freq1, freq2)
            )
